...
def CPLEX_Solver(label, m, edge, ILP = True):
    """         
    ...
    prob, x_idx = problem_setup(label, edge, ILP)
    ...
    return np.round(sol.get_values(list(x_idx)), decimals=5)

def problem_setup(label, edge, ILP):
    """         
    ...
    for (i, j) in edge:
        ...
        prob.linear_constraints.set_linear_components('violation', [[f"z_{i}{j}"], [1.0]])
        prob.linear_constraints.add(
