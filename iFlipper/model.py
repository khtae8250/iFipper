...
from utils import performance_func

class Model:
        def __init__(self, model_type, x_train, y_train, w_train, edge_train, x_test, y_test, w_test, edge_test, x_val, y_val):
            """         
                Args: 
        ...
        def train_lr(self, C_arr):
            ...
            for random_state in self.random_state_arr:
                ...
                train_result = performance_func(clf, self.x_train, self.y_train, self.w_train, self.edge_train)
                train_acc += train_result[0] / len(self.random_state_arr)
                train_cs += train_result[1] / len(self.random_state_arr)

                test_result = performance_func(clf, self.x_test, self.y_test, self.w_test, self.edge_test)
